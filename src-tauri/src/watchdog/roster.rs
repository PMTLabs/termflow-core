//! The list of terminals a Watchdogs rule could watch — not `spawn_pipeline_watchdog`.
//!
...
//! missing — filled from that rule's `watchdog_targets` snapshot, so a closed terminal still shows its

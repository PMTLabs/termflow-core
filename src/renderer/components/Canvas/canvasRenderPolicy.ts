...
/**
 * A stable key for a desired-policy map: the sorted list of ids that want WebGL.
 *
 * `assignTiers` produces a FRESH object on every viewport change, and a pan dispatches
 * one of those per pointer-move — so an effect keyed on the tier map alone re-runs at
 * frame rate. Almost every one of those frames wants exactly the same policies, and
 * reconciling on the rest would create and dispose real GPU contexts at 60fps as nodes
 * cross the cull boundary. Comparing this key instead makes the no-op frames free.
 *
 * Only `'webgl'` entries matter: a policy map is total over the workspace, so the `'dom'`
 * ids are the complement and carry no extra information.
 */
export function policySignature(desired: Record<string, RenderPolicy>): string {
  return Object.keys(desired).filter((id) => desired[id] === 'webgl').sort().join('\u0000');
}
